<<<<<<<< HEAD:old py code/Persuasion29.py
...
def timing_bullseye_chase(rounds=3, escape_distance=3):
    print("A chase begins! Press SPACE when the '*' is in the center (bullseye)!")
    ...
    while True:
        ...
        while time.time() - start_time < 3:  # 3 seconds per round
            display = "|" + "".join("-" if i != pos else "*" for i in range(1, 22)) + "|"
...
def skill_check_combat(enemy_name, enemy_difficulty=None, stat=None):
    ...
    while player_stamina > 0 and enemy_stamina > 0:
        ...
        print("[A] Attack")
        print("[U] Use an item")
        print("[F] Flee (if available)")
        print("[S] Show stats")
        print("[Q] Quit combat (forfeit)")
        ...
        if action == "u":
            ...
            delay_print("You forfeit the fight and retreat in disgrace.")
            game_state["score"] -= 10
            wait_for_space()
            describe_room()
            return
        elif action == "f" and transformed:
        ...
        # Default to attack if 'a' or anything else
...
>>>>>>>> 7dec0e0e2b02c77e5bcc74dd480b6cfef4de6220:Persuasion29.py
