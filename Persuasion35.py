                ...
                while True:
                    ...
                    if not info_shown and choice in {"a", "m", "t", "s"}:
                        ...
                        if effective_weight == 1:
                            delay_print(f"{suspect['name']}'s alibi seems very shaky.")
                        elif effective_weight == 2:
                            delay_print(f"{suspect['name']}'s alibi is questionable.")
                        elif effective_weight == 3:
                            delay_print(f"{suspect['name']}'s alibi seems fairly solid.")
                        else:
                            delay_print(f"{suspect['name']}'s alibi is very convincing.")
                    ...
                    def check_tolerance():
                        suspect["tolerance"] = suspect.get("tolerance", game_state.get("suspect_tolerance", 5))
                        suspect["tolerance"] -= 1
                        if suspect["tolerance"] == 2:
                            delay_print(f"{suspect['name']} is growing agitated. You sense danger.")
                        # Add random buffer: after tolerance reaches 0, there's a 50% chance each time to snap
                        if suspect["tolerance"] <= 0:
                            if random.random() < 0.5:  # 50% chance to snap each time after threshold
                    ...
                    if choice == "p" and "p" in options:
                        ...
                        if roll > 12:
                            ...
                            if suspect.get("is_murderer"):
                                ...
                                delay_print(f"{suspect['name']} reveals a detail that points suspicion elsewhere.")
                                unused_clues = [c for c in game_state["clue_pool"] if c not in game_state["clues"]]
                                if unused_clues:
                                    clue = random.choice(unused_clues)
                                    game_state["clues"].append(clue)
                                    delay_print(f"{suspect['name']} mentions something odd: {clue}")
                        ...
                        if clue_choice.isdigit():
                            ...
                            if 0 <= clue_idx < len(game_state["clues"]):
                                ...
                                if game_state["clue_motives"].get(clue) == suspect["motive"]:
                                    ...
                                    delay_print(f"{suspect['name']} stammer and contradict themselves. You sense they're hiding something.")
                                    game_state["score"] += 15
