    ...
    while True:
        ...
        if user_input.isdigit():
            ...
            if 0 <= idx < len(suspects_here):
                ...
                # --- Track clues presented to this suspect ---
                ...
                while True:
                    ...
                    if game_state["faith"] >= 14:
                        ...
                        options.add("12")
                    # Persuasion option: only if stat requirement met and not already attempted
                    if (
                        ...
                        options.add("p")
                        options.add("persuade")
                    # --- ORANGE for [14] only if there are clues not yet presented to this suspect ---
                    unpresented_clues = [clue for clue in game_state.get("clues", []) if clue not in suspect["presented_clues"]]
                    if game_state.get("clues"):
                        if unpresented_clues:
                            print(f"{ORANGE}[14] Present a clue as evidence{RESET}")
                        else:
                            print("[14] Present a clue as evidence")
                        options.add("14")
                    ...
                    def check_tolerance():
                        ...
                        if suspect["tolerance"] <= 0:
                            if random.random() < 0.5:
                                delay_print(personalize_response(f"{suspect['name']} snaps! They attack you in a fit of rage!", suspect))
                                wait_for_space()
                                skill_check_combat(suspect["name"])
                    ...
                    if choice == "1":
                        ...
                        delay_print(f"{suspect['name']} says: '{personalize_response(rel, suspect)}'")
                        game_state["journal"].append(
                        ...
                        # Only allow if stat requirement is met
                        if game_state.get("comeliness", 0) < 14 and game_state.get("perception", 0) < 14:
                            ...
                            wait_for_space()
                            continue
                        # Restrict to one attempt per suspect
                        ...
                        # --- FUDGE Skill Check for Persuasion ---
                        ...
                        if skill_total >= target:
                            ...
                            suspect["tolerance"] = game_state.get("suspect_tolerance", 5)
                            suspect["persuaded"] = True  # Mark as persuaded

                            delay_print(f"{YELLOW}{suspect['name']}'s tolerance for questioning has been restored!{RESET}")

                            # --- Hypnotic Minigame ---
                            delay_print(f"{ORANGE}You attempt a hypnotic suggestion... Focus!{RESET}")
                            if timing_bullseye_chase(rounds=1, mode="boss"):
                                # If this is the culprit, they confess and you win
                                if suspect.get("is_murderer"):
                                    delay_print(f"{YELLOW}Under your suggestion, {suspect['name']} breaks down and confesses!{RESET}")
                                    game_state["forced_accused"] = suspect["name"]
                                    if "Master Hypnotist" not in game_state["achievements"]:
                                        game_state["achievements"].add("Master Hypnotist")
                                        delay_print(f"{CYAN}Achievement unlocked: Master Hypnotist!{RESET}")
                                    case_resolution()
                                    return
                                else:
                                    # Not the culprit: random benefit, each with its own bullseye
                                    benefits = [
                                        "Reveal a Hidden Clue",
                                        "Permanently Max Tolerance)",
                                        "Unlock Unique Backstory",
                                        "Temporary +2 Perception/Sanity (3 turns)"
                                    ]
                                    benefit = random.choice(benefits)
                                    delay_print(f"{ORANGE}You feel your hypnotic suggestion drifting... fate will decide the boon!{RESET}")
                                    delay_print(f"{ORANGE}Attempting: {benefit}{RESET}")
                                    if timing_bullseye_chase(rounds=1, mode="boss"):
                                        if benefit == "Reveal a Hidden Clue":
                                            hidden = [(pos, clue) for pos, clue in clue_locations.items() if clue not in game_state["clues"]]
                                            if hidden:
                                                pos, clue = random.choice(hidden)
                                                room = game_state["visited_locations"].get(pos, "Unknown Room")
                                                delay_print(f"{CYAN}In trance, {suspect['name']} mutters: 'Seek {clue} in the {room}...'{RESET}")
                                                game_state["journal"].append(f"Hypnotic clue: {clue} in {room}")
                                            else:
                                                delay_print(f"{CYAN}No hidden clues remain!{RESET}")
                                        elif benefit == "Permanently Max Tolerance (prevents transformation)":
                                            suspect["tolerance"] = 99
                                            suspect["no_transform"] = True
                                            delay_print(f"{CYAN}{suspect['name']}'s mind is calmed. They will never become hostile.{RESET}")
                                            game_state["journal"].append(f"Hypnosis: {suspect['name']} cannot transform.")
                                        elif benefit == "Unlock Unique Backstory":
                                            backstories = [
                                                "I saved the Bishop from the flames. I still remember the heat and the smoke — the way the stained glass glowed like molten eyes. I was barely tall enough to reach the brass handles on the vestry doors, but I heard his screams. Something ancient in me moved. I dragged the Bishop out by his robes while the fire licked at his cassock. He survived… barely. But from that day forward, he never looked at me as a child again — only as an omen.",
                                                "I wrote letters to a Forbidden Cult. It started with a letter — hidden between the pages of an old hymnal. The handwriting curled like vines, whispering secrets in ink I didn’t recognize. I answered it. I shouldn’t have. The replies came faster, stranger, and I began to crave the riddles. I never saw their faces. But I swear their thoughts tangled with mine like roots beneath the manor.",
                                                "I lost My sister to the Manor’s Curse. I warned her not to wander off, not past the east wing — where the mirrors don’t reflect right. But she laughed, said she heard music. Then she was gone. Not dead, not missing — just… erased. I found her ribbon by a door that wasn’t there the day before. I still dream of her whispering through the walls.",
                                                "I was taken in… but never truly belonged. They say I was found wrapped in cloth embroidered with symbols no one could translate. No one ever claimed me. The man who raised me said I was left in the chapel, like a forgotten relic. But I know the truth now: I was delivered. Chosen by something older than the family name carved above the hearth.",
                                                "I see lives that were never mine. They aren’t dreams — they’re visions, etched into my mind like ink soaked into paper. I walk through the manor and feel its past cling to me. I speak names I shouldn’t know, open doors I’ve never seen before. Sometimes I wake with soot on my hands or hear footsteps pacing above me when I’m alone. The house is showing me what it remembers… and what it wants me to become.",
                                            ]
                                            story = random.choice(backstories)
                                            delay_print(f"{GREY}Under trance, {suspect['name']} reveals: {story}\033[0m{RESET}")
                                            game_state["journal"].append(f"Hypnotic backstory: {suspect['name']} — {story}")
                                        elif benefit == "Temporary +2 Perception/Sanity (3 turns)":
                                            delay_print(f"{ORANGE}Focus for the temporary buff...{RESET}")
                                            if timing_bullseye_chase(rounds=1, mode="boss"):
                                                game_state["temp_buff_turns"] = 3
                                                game_state["temp_buff"] = {"perception": 2, "sanity": 2}
                                                delay_print(f"{ORANGE}You feel your mind sharpen and your nerves steady! (+2 Perception/Sanity for 3 turns){RESET}")
                                                game_state["journal"].append("Hypnotic buff: +2 Perception/Sanity for 3 turns.")
                                            else:
                                                delay_print(f"{RED}The suggestion fails to take hold. No buff granted.{RESET}")
                                        if "Master Hypnotist" not in game_state["achievements"]:
                                            game_state["achievements"].add("Master Hypnotist")
                                            delay_print(f"{CYAN}Achievement unlocked: Master Hypnotist!{RESET}")
                                    else:
                                        delay_print(f"{RED}The suggestion fails to take hold. No benefit granted.{RESET}")
                            else:
                                delay_print(f"{RED}The suggestion fails to take hold. No secrets are revealed.{RESET}")
                        else:
                        ...
                        if game_state.get("clues"):
                            ...
                            if clue_choice.isdigit():
                                ...
                                if 0 <= clue_idx < len(game_state["clues"]):
                                    ...
                                    # Mark this clue as presented to this suspect
                                    suspect["presented_clues"].add(presented_clue)
                                    # Show only ORANGE for [14] if there are still unpresented clues
                                    ...
                                    delay_print(f"{YELLOW}You present the clue: {short_clue}{RESET}")
...
def intro_scene():
    ...
    while True:
        ...
        #     wait_for_space()
        # elif user_input == "3":
        #     if not game_state.get("prayed_outside_manor", False):
        #         game_state["faith"] += 1
        #         game_state["prayed_outside_manor"] = True
        #         delay_print("You pause and offer a silent prayer. You feel your faith strengthen (+1 Faith).")
        #     else:
        #         delay_print("You have already prayed here. Further prayers bring no additional comfort.")
        #     wait_for_space()
        # elif user_input == "2":
        #     character_creation()
            break
