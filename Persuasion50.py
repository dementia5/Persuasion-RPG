...
def timing_bullseye_chase(rounds=3, escape_distance=4):
    """
    A chase minigame: Press SPACE when the red circle (●) is in the bullseye (◎)!
    If you escape all monstrosities, unlock the "Evaded every monstrosity" achievement.
    """
        ...
        if os.path.exists("savegame.json"):
            ...
            if "suspects_interrogated" in game_state:
                if isinstance(game_state["suspects_interrogated"], list):
                    game_state["suspects_interrogated"] = set(str(item) for item in game_state["suspects_interrogated"])
...
def show_journal():
    ...
    if responses:
        ...
        print("-----------------------------")
            ...
            # Separator before quest entries
    print("\n--- THINGS TO DO ---")
    for quest in game_state["quests"]:
        # Only show quests that are not completed (not strikethrough)
...
def use_item(context=None, return_func=None):
    ...
    # Only highlight the Magnum if it's in inventory and not yet used in this combat
    magnum_in_inventory = "Loaded Magnum .44" in game_state["inventory"]
    ...
    if from_combat and magnum_in_inventory and (is_monstrosity or suspect):
        combat_highlight.add("Loaded Magnum .44")
    ...
    if choice.isdigit():
        ...
        if 0 <= idx < len(usable):
            ...
            if artifact:
                # Apply main effect
                if artifact["name"] == "Loaded Magnum .44":
                    bullets = artifact.get("bullets", 6)
                    if from_combat and suspect:
                        if is_monstrosity:
                            if bullets > 0:
                                delay_print(f"{YELLOW}You fire the Magnum .44 at the monstrosity!{RESET}")
                                ...
                                delay_print(f"{RED}The Magnum .44 is empty!{RESET}")
                        else:
                            if bullets > 0:
                                delay_print(f"{YELLOW}You fire the Magnum .44 at your foe!{RESET}")
                                artifact["bullets"] -= 1
                        ...
                        delay_print(f"{RED}You brandish the Magnum .44, but there is no foe to fire upon.{RESET}")
                    wait_for_space()
...
def skill_check_combat(enemy_name, enemy_difficulty=None, stat=None):
    ...
    if suspect and transformed and player_stamina > 0 and enemy_stamina <= 0:
        used_magnum = game_state.get("combat_bonus", 0) >= 10
        used_amulet = "Abraxas Amulet" in game_state["inventory"]
        if not used_magnum and not used_amulet:
...
def show_score():
    ...
    if game_state["sanity"] > 10:
            if "Kept sanity above 10" not in game_state["achievements"]:
                game_state["achievements"].add("Kept sanity above 10! Sane as it ever was")
                game_state["score"] += ACHIEVEMENT_SCORE
                print(f"{CYAN}Achievement unlocked: {ACHIEVEMENTS['Kept sanity above 10! Sane as it ever was']} (+{ACHIEVEMENT_SCORE} points){RESET}")
    ...
    if len(game_state["clues"]) >= game_state.get("required_clues", 5):
        ...
        if accused.get("is_murderer"):
            ...
            if required_pages.issubset(player_pages):
                ...
                mark_quest_completed("Assemble the Book of Eibon and open the portal to the Elder Ones' realm.")  # <-- Place here
                final_boss_sequence()
                return
            else:
