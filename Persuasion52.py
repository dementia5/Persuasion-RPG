...
def show_accusation_breakdown(accused, correct_culprit, clue_motives, player_clues):
    ...
    if accused_motive == culprit_motive:
        print(f"- Motive: {YELLOW}{accused_motive}{RESET} (matches the real culprit)")
    else:
        print(f"- Motive: {RED}{accused_motive}{RESET} (real culprit's motive: {YELLOW}{culprit_motive}{RESET})")

    # Means
    accused_means = accused.get("means", "Unknown")
    culprit_means = correct_culprit.get("means", "Unknown")
    if accused_means == culprit_means:
        print(f"- Means: {YELLOW}{accused_means}{RESET} (matches the real culprit)")
    else:
        print(f"- Means: {RED}{accused_means}{RESET} (real culprit's means: {YELLOW}{culprit_means}{RESET})")

    # Opportunity
    accused_opp = accused.get("opportunity", "Unknown")
    culprit_opp = correct_culprit.get("opportunity", "Unknown")
    if accused_opp == culprit_opp:
        print(f"- Opportunity: {YELLOW}{accused_opp}{RESET} (matches the real culprit)")
    else:
        print(f"- Opportunity: {RED}{accused_opp}{RESET} (real culprit's opportunity: {YELLOW}{culprit_opp}{RESET})")
