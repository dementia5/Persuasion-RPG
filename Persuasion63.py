...
def debug_add_artifact():
    ...
    while True:
        ...
        choice = input("Select artifact(s) by number or name, or press Enter to return: ").strip()
...
def show_murder_board():
    ...
    for time in times:
        ...
        print(row)
    # Show alibis for each suspect in easy mode
    if easy_mode:
        print(f"\n{CYAN}Alibis:{RESET}")
        for s in game_state["suspects"]:
        ...
        # Show trust states for each suspect
        print("\nTrust Levels:")
        for s in game_state["suspects"]:
            if s["name"] in interrogated_names:
                trust = s.get("trust", 50)
                print(f"  {s['name']}: {trust_state(trust)} ({trust}/100)")
        print("\nSuspects with high trust (Trusting/Loyal) are more likely to tell the truth or help you challenge alibis.")
        print("Suspects with low trust (Hostile/Distrustful) may lie or mislead you.")
    else:
...
def adjust_trust(suspect, amount):
    suspect["trust"] = max(0, min(100, suspect.get("trust", 50) + amount))
...
def interrogate_suspect():
    ...
    while True:
        ...
        for i, s in enumerate(suspects_here):
            print(f"[{i+1}] {s['name']} (Trust: {trust_state(s.get('trust', 50))} [{s.get('trust', 50)}/100])")
        ...
        if user_input.isdigit():
            ...
            if 0 <= idx < len(suspects_here):
                ...
                while True:
                    ...
                    print("[13] Complete a favor/side quest")
                    ...
                    # 1. Persuasion (trust up/down)
                    if choice == "1":
                        persuasion_roll = random.randint(1, 20)
                        if persuasion_roll + game_state.get("comeliness", 0) > 12:
                            delay_print(f"{GREEN}{suspect['name']} seems convinced by your words. Their trust increases.{RESET}")
                            adjust_trust(suspect, +10)
                        else:
                            delay_print(f"{RED}{suspect['name']} is unimpressed. Trust decreases.{RESET}")
                            adjust_trust(suspect, -5)
                        wait_for_space()
                        response_count += 1
                        continue

                    # 2. Honest or kind actions (trust up)
                    elif choice == "2":
                        delay_print(f"{GREEN}Your honesty/kindness earns {suspect['name']}'s trust.{RESET}")
                        adjust_trust(suspect, +5)
                        wait_for_space()
                        response_count += 1
                        continue

                    # 3. Ask alibi (lie if trust low)
                    elif choice == "3":
                        if suspect.get("trust", 50) <= 19:
                            fake_alibi = "I was in the attic with the Bishop all night. You can ask anyone."
                            ...
                            alibi_text = alibi.get("text", str(alibi)) if isinstance(alibi, dict) else str(alibi)
                            delay_print(f"{suspect['name']} says: '{alibi_text}'")
                        wait_for_space()
                        response_count += 1
                        continue

                    # 4. Ask who they suspect (lie if trust low, aggressive if pressed)
                    elif choice == "4":
                        if suspect.get("trust", 50) <= 19:
                        ...
                        # High trust: allow dismantling another suspect's alibi
                        if suspect.get("trust", 50) >= 60:
                            ...
                            if sub_choice.isdigit():
                                ...
                                if 0 <= idx < len(others):
                                    ...
                                    wait_for_space()
                        # Aggressive press (simulate with random chance)
                    ...
                    # 9. Ask about secret relationships
                    elif choice == "9":
                    ...
                    # 10. Ask about blackmail or occult matters
                    elif choice == "10":
                    ...
                    # 11. Direct accusation (aggressive, trust down)
                    elif choice == "11":
                        delay_print(f"{RED}{suspect['name']} is offended by your accusation. Trust decreases.{RESET}")
                        adjust_trust(suspect, -7)
                    ...
                    # 13. Complete a favor/side quest (trust up)
                    elif choice == "13":
                        delay_print(f"{YELLOW}{suspect['name']} is grateful for your help. Trust increases significantly!{RESET}")
                        adjust_trust(suspect, +15)
                        wait_for_space()
                        response_count += 1
                        continue

                    # 14. Present a clue as evidence (trust up/down based on clue)
                    elif choice == "14":
                        if game_state.get("clues"):
                            ...
                            if clue_choice.isdigit():
                                ...
                                if 0 <= clue_idx < len(game_state["clues"]):
                                    ...
                                    # High trust: instant confession if culprit and clue matches motive
                                        ...
                                        case_resolution()
                                        return
                                    # 4. Presenting evidence matching motive (not culprit)
                                    ...
                                    # 7. Presenting irrelevant or accusatory clues
...
def dream_flashback():
    ...
    delay_print(f"\033[3mA chapel cloaked in shadow, the child's voice echoing like a song half-remembered. A locked door pulses in the back of my mind like a heartbeat.{RESET}")
    delay_print(f"\033[3mWhispers echo backward through time and thought. I wake gasping, the image of a burning sigil fading from my vision.{RESET}")
    print()
    delay_print("- note discovered on the floor near the chapel, signed by Bishop Alaric Greaves")
    wait_for_space()
    intro_scene()
