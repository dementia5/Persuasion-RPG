...
def show_inventory():
    ...
    if game_state["inventory"]:
        for idx, item in enumerate(game_state["inventory"], 1):
         ...
         # Special option for Book of Eibon
        if "Book of Eibon" in game_state["inventory"]:
            print(f"[U] Use the Book of Eibon")
 ...
 # Book of Eibon special use
        if choice.strip().lower() == "u" and "Book of Eibon" in game_state["inventory"]:
            use_book_of_eibon()
            return
...
def use_book_of_eibon():
    ...
    complete = required_pages.issubset(player_pages)
    clear()
...
def evoke_book_of_eibon():
    ...
    delay_print(f"{RED}The Book of Eibon trembles in your hands. A portal yawns wide, swirling with impossible colors and the sound of distant, maddening reeds...{RESET}")
    wait_for_space()
    final_boss_sequence()
...
def magical_pinwheel_animation():
    ...
    for i in range(3):  # Repeat the swirl a few times
        for frame in frames:
            ...
            time.sleep(0.13)
    os.system('cls' if os.name == 'nt' else 'clear')
    print("\n" * 3 + " " * 10 + "\033[95mTHE PORTAL SWIRLS OPEN...\033[0m\n" * 1)
    time.sleep(1.2)
...
def character_creation():
    ...
    show_initialization_progress()  # Show initialization progress
