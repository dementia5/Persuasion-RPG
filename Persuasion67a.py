...
# import colorama
# colorama.init()
import tkinter as tk # For future GUI elements
from tkinter import messagebox

# from Persuasion67 import render_map
# import pygame
# pygame.mixer.init()
...
# pygame.mixer.music.load(resource_path("title.mp3"))
# pygame.mixer.music.play()  # Loop never
...
def init_music():
    global music_enabled, music_initialized
    if music_initialized:
        return
    try:
        # Suppress pygame's greeting output
        import io
        import sys
        import os
        ...
        # Prompt while music is playing
        choice = input("Background music is ON. Turn music OFF? (Y/N): ").strip().lower()
        if choice == "y":
            pygame.mixer.music.stop()
            music_enabled = False
    except Exception as e:
        print(f"Music could not be started: {e}")
        music_enabled = False
...
one_use_artifacts = {
    "Brass Spyglass": "You peer through the spyglass, but it reveals nothing new.",
    "Sealed Letter": "You have already read the letter; its secrets are spent.",
    "Tarot Deck": "The cards are spent and reveal no further insight.",
    "Ritual Candle": "The candle has burned down to a stub.",
    "Whispering Skull": "The skull is silent now.",
    "Tattered Map": "The map is worn and unreadable.",
    "Phial of Holy Water": "The vial is empty.",
    "Pocket Watch": "The watch ticks, but nothing happens.",
    "Candle Snuffer": "The snuffer is cold and inert.",
    "Bloodstained Handkerchief": "The handkerchief is stained and offers no more clues.",
    "Strange Key": "The key has already been used.",
    "Silver Ornate Key": "The key has already been used."
}
...
def delay_print(s, speed=0.0003, width=80):
    """Prints text slowly, wrapping lines to the given width."""
    import threading
    import msvcrt

    # Wrap the text before printing
...
def process_trust_sequence(suspect, choice):
    """
    Updates the suspect's trust based on the player's question sequence.
    Rewards following the ideal sequence, penalizes breaking it.
    """
    TRUST_SEQUENCE = ["1", "2", "3", "5", "6"]
    if "question_sequence" not in suspect:
        suspect["question_sequence"] = []
    if choice in TRUST_SEQUENCE:
        suspect["question_sequence"].append(choice)
        seq = suspect["question_sequence"]
        if seq == TRUST_SEQUENCE[:len(seq)]:
            if len(seq) == len(TRUST_SEQUENCE):
                delay_print(f"{YELLOW}Your careful approach has built deep trust with {suspect['name']}!{RESET}")
                adjust_trust(suspect, +15)
                suspect["question_sequence"] = []  # Reset after full sequence
            else:
                adjust_trust(suspect, +2)
        else:
            delay_print(f"{RED}You sense {suspect['name']} growing wary. Trust drops and rapport is lost!{RESET}")
            adjust_trust(suspect, -7)
            suspect["question_sequence"] = []
    else:
        # Do not reset sequence if the question is not in TRUST_SEQUENCE
        pass

def personalize_response(text, suspect):
# Replace 'Their'/'their' with "Name's" (possessive)
    text = text.replace("Their", f"{suspect['name']}'s")
    text = text.replace("their", f"{suspect['name']}'s")
    # Optionally, handle 'They'/'they' and 'them, just in case'
    text = text.replace("They", suspect['name'])
    text = text.replace("they", suspect['name'])
    text = text.replace("them", suspect['name'])
    return text
...
def debug_add_artifact():
    """Debug: Add any artifact(s) to inventory by name or number. Stays on menu until Return."""
    while True:
        clear()
        ...
        print(f"[A] Add ALL artifacts")
        print(f"[{len(artifact_pool)+1}] Return")
        print("You may enter multiple numbers or names separated by commas (e.g. 1,3,Book of Eibon)")
        choice = input("Select artifact(s) by number, name, [A] for all, or press Enter to return: ").strip()
        if not choice or choice == str(len(artifact_pool)+1):
            break

        if choice.lower() == "a":
            added = 0
            for artifact in artifact_pool:
                if artifact["name"] not in game_state["inventory"]:
                    game_state["inventory"].append(artifact["name"])
                    added += 1
            print(f"Added all artifacts to inventory. ({added} added)")
            wait_for_space()
            continue
        ...
        for entry in entries:
            if entry.isdigit():
                ...
                if artifact_name not in [a["name"] for a in artifact_pool]:
                    print(f"Invalid artifact name: {artifact_name}")
                    continue
...
def initialize_suspects():
    ...
    # --- Assign believes_culprit field ---
    real_culprit_name = game_state["murderer"]
    for suspect in game_state["suspects"]:
        suspect["believes_culprit"] = None
    # Pick one non-culprit (or the culprit themselves) to know the truth
    non_culprits = [s for s in game_state["suspects"] if not s.get("is_murderer")]
    if non_culprits:
        witness = random.choice(non_culprits)
        witness["believes_culprit"] = real_culprit_name
    else:
        # Fallback: culprit knows themselves
        murderer = next(s for s in game_state["suspects"] if s.get("is_murderer"))
        murderer["believes_culprit"] = real_culprit_name
    ...
    # Assign two clues from the clue pool to random suspects for trust-based reveal
    if "clue_pool" in game_state and len(game_state["clue_pool"]) >= 2:
        trust_clues = random.sample(game_state["clue_pool"], 2)
        suspects = random.sample(game_state["suspects"], 2)
        for suspect, clue in zip(suspects, trust_clues):
            suspect["trust_clue"] = clue
    else:
        for suspect in game_state["suspects"]:
            suspect["trust_clue"] = None
...
#     "name": "Scrying Lens",
#     "desc": (
#         "A cloudy crystal lens set in tarnished silver. When held to the eye, it reveals the hidden movements of those who dwell within the manor (check map)."
#     )
# }
...
artifact_pool = [
    {
        "name": "Scrying Lens",
        "desc": "A cloudy crystal lens set in tarnished silver. When held to the eye, it reveals the hidden movements of those who dwell within the manor (check map).",
    },
    {
    ...
    },
    {
        "name": "Dreaming Note",
        "desc": (
            "I am dreaming... or remembering. A shiver creeps down my spine as my mind\n"
            "conjures the weight of memory.\n"
            "A chapel cloaked in shadow, the child's voice echoing like a song half-remembered. "
            "A locked door pulses in the back of my mind like a heartbeat.\n"
            "Whispers echo backward through time and thought. I wake gasping, the image of a burning sigil fading from my vision."
        ),
    },
    {
        ...
        {
        "name": "Locked Chest",
        "desc": "A sturdy wooden chest with a heavy iron lock. It looks like it could contain something valuable.",
        "type": "chest",
        "content": None  # Will be filled during assignment
    },
    {
        "name": "Ornate Chest",
        "desc": "An ornate chest inlaid with silver filigree. The lock is intricate and old.",
        "type": "chest",
        "content": None
    },
    {
        "name": "Book of Eibon",
        "desc": "A legendary tome of forbidden knowledge, its pages scattered and power incomplete."
    },
    {
        "name": "Silver Ornate Key",
        "desc": "A beautifully crafted silver key, cold to the touch. It looks important.",
        "effect": "unlock",
        "amount": 1  # Unlocks a chest
    },
    {
        "name": "Abraxas Amulet",
    ...
    },
    {
        ...
        "desc": "A heavy gold ring engraved with ecclesiastical symbols. When used, it reveals patterns in suspects’ alibis.",
        "effect": "puzzle",
...
def assign_blackmail_and_favors():
    suspects = game_state["suspects"]
    clue_pool = game_state.get("clue_pool", [])
    artifact_names = [a["name"] for a in artifact_pool if a.get("type") != "chest"]

    # Randomly pick 3 suspects for leverage
    leverage_suspects = random.sample(suspects, min(3, len(suspects)))
    culprit = next((s for s in suspects if s.get("is_murderer")), None)

    for suspect in leverage_suspects:
        # Decide randomly: blackmail or favor
        if random.choice([True, False]):
            # Assign blackmail: pick a clue not already used, ideally linked to culprit
            possible_clues = [c for c in clue_pool if c not in suspect.get("presented_clues", set())]
            if culprit and suspect is culprit:
                # Try to pick a clue matching culprit's motive
                motive = culprit.get("motive")
                motive_clues = [c for c in possible_clues if game_state.get("clue_motives", {}).get(c) == motive]
                clue = random.choice(motive_clues) if motive_clues else (random.choice(possible_clues) if possible_clues else None)
            else:
                clue = random.choice(possible_clues) if possible_clues else None
            if clue:
                suspect["blackmail"] = clue
                suspect["blackmailed"] = False
        else:
            # Assign favor: pick an artifact not already in inventory
            possible_artifacts = [a for a in artifact_names if a not in game_state.get("inventory", [])]
            artifact = random.choice(possible_artifacts) if possible_artifacts else None
            if artifact:
                suspect["favor_item"] = artifact
                suspect["favor_pending"] = True

# Call this after suspects are initialized
assign_blackmail_and_favors()
...
def assign_clues_to_rooms():
    """Assign each clue to a random unique room position at game start."""
    global clue_locations
    clue_locations = {}
    # Only use positions of described rooms (not "Empty Room")
    possible_positions = [pos for pos, room in game_state["visited_locations"].items() if room != "Empty Room"]
    random.shuffle(possible_positions)
    for clue, pos in zip(game_state["clue_pool"], possible_positions):
        clue_locations[pos] = clue

def assign_artifacts_to_rooms():
    possible_rooms = [r for r in room_templates.keys() if r != "Foyer"]
    random.shuffle(possible_rooms)
     # Prepare chest contents
    chest_contents = []
    available_clues = [c for c in game_state.get("clue_pool", [])]
    available_artifacts = [a for a in artifact_pool if a.get("type") != "chest"]
    if available_clues:
        chest_contents += random.sample(available_clues, min(2, len(available_clues)))
    if available_artifacts:
        chest_contents += [a["name"] for a in random.sample(available_artifacts, min(2, len(available_artifacts)))]
    # Assign chests with contents
    chest_artifacts = [a for a in artifact_pool if a.get("type") == "chest"]
    for chest, content, room in zip(chest_artifacts, chest_contents, possible_rooms):
        chest["content"] = content
        game_state.setdefault("artifact_locations", {})[room] = dict(chest)
...
def reset_game_state():
    ...
    game_state["suspects"] = []
    game_state["location"] = ""
    game_state["enough_evidence_shown"] = False
    game_state["turns"] = 0
...
def move_to_new_room(direction=None, show_room=True):
    ...
    if "Brass Spyglass" in game_state["inventory"]:
        pos = game_state["position"]
        clue = clue_locations.get(pos)
        if clue and clue not in game_state["clues"]:
            game_state["clues"].append(clue)
            delay_print(f"{YELLOW}The spyglass glimmers. You spot something unusual: {clue}{RESET}")
            game_state["journal"].append(f"CLUE FOUND (Spyglass): {clue}")
...
def show_journal():
    ...
    # Group testimonies by suspect
    testimonies = [entry for entry in game_state["journal"] if entry.startswith("Testimony from ")]
    other_entries = [entry for entry in game_state["journal"] if not (
        entry.startswith("CLUE FOUND") or entry.startswith("ARTIFACT FOUND") or entry.startswith("Testimony from "))]

    # --- Clues ---
    if clues:
        print(f"\n{YELLOW}--- Clues Discovered ---{RESET}")
        for entry in clues:
            print(f"🕵️‍♂️ {entry}")

    # --- Artifacts ---
    if artifacts:
        print(f"\n{CYAN}--- Artifacts Discovered ---{RESET}")
        for entry in artifacts:
            print(f"📖 {entry}")

    # --- Testimonies grouped by suspect ---
    if testimonies:
        print(f"\n{ORANGE}--- Suspect Testimonies ---{RESET}")
        # Build a dict: suspect_name -> list of testimonies
        suspect_testimonies = {}
        for entry in testimonies:
            # Extract suspect name
            try:
                suspect_name = entry.split("Testimony from ")[1].split(" at ")[0]
            except Exception:
                suspect_name = "Unknown"
            suspect_testimonies.setdefault(suspect_name, []).append(entry)
        # Print testimonies grouped by suspect
        for suspect in sorted(suspect_testimonies.keys()):
            print(f"\n{GREEN}{suspect}:{RESET}")
            for t in suspect_testimonies[suspect]:
                # Optionally, strip redundant "Testimony from X at Y: " prefix for brevity
                try:
                    detail = t.split(":", 2)[-1].strip()
                except Exception:
                    detail = t
                print(f"  - {detail}")
    ...
    # --- Other entries ---
    if other_entries:
        print(f"\n{GREY}--- Other Events ---{RESET}")
        for entry in other_entries:
            print(entry)

    # --- Social Status: Trust, Blackmail, Favors ---
    print(f"\n{CYAN}--- Suspect Social Status ---{RESET}")
    for s in game_state["suspects"]:
        trust = s.get("trust", 50)
        trust_str = trust_state(trust)
        status = []
        if s.get("blackmailed"):
            status.append("🔒")
        if s.get("favor_pending"):
            status.append("🤝")
        status_str = " ".join(status) if status else "—"
        print(f"{s['name']}: Trust: {trust_str} [{trust}/100] | {status_str}")

    # --- Quests ---
    print(f"\n{CYAN}--- THINGS TO DO ---{RESET}")
    for quest in game_state["quests"]:
        if not quest.startswith("\033[9m"):
            print(quest)

    if not clues and not artifacts and not testimonies and not other_entries:
...
def show_murder_board():
    ...
    for s in game_state["suspects"]:
        ...
        time = alibi.get("time", "Unknown")
        loc = alibi.get("location", "Unknown")
        alibi_text = alibi.get("text", "")
        timeline.setdefault(time, {}).setdefault(loc, []).append(s["name"])
        alibi_map[s["name"]] = alibi_text
        # Leverage icons
        leverage = []
        if s.get("blackmailed"):
            leverage.append("🔒")
        if s.get("favor_pending"):
            leverage.append("🤝")
        leverage_map[s["name"]] = " ".join(leverage) if leverage else "—"
        # Trust
        trust = s.get("trust", 50)
        trust_map[s["name"]] = trust
    ...
    # --- Prepare columns and formatting ---
    times = sorted(timeline.keys())
    locations = sorted({loc for t in timeline.values() for loc in t})
    ...
    # --- Print timeline table with Leverage column ---
    print("Time      | " + " | ".join(f"{loc[:col_width]:<{col_width}}" for loc in locations) + f" | {'Leverage':<10}")
    print("-" * (11 + len(locations) * (col_width + 3) + 13))
    for time in times:
        row = f"{time:<9} | "
        row += " | ".join(
        ...
        # For each time, show leverage for each suspect present at that time (first in cell)
        suspects_in_row = []
        for loc in locations:
            suspects_in_row.extend(timeline[time].get(loc, []))
        leverage_str = " ".join([leverage_map.get(name, "—") for name in suspects_in_row]) or "—"
        row += f" | {leverage_str:<10}"
        print(row)

    # --- Easy mode: Show alibis, trust, and legend ---
    if easy_mode:
        print(f"\n{CYAN}Alibis:{RESET}")
        for s in game_state["suspects"]:
            if s["name"] in interrogated_names:
                alibi_text = alibi_map.get(s["name"], "No alibi provided.")
                print(f"  {YELLOW}{s['name']}{RESET}: {alibi_text}")

        print("\nLegend: Each cell shows interrogated suspects who claim to be in that room at that time.")
        print("Leverage: 🔒 = Blackmailed, 🤝 = Owes Favor")
        ...
        for s in game_state["suspects"]:
            if s["name"] in interrogated_names:
                trust = s.get("trust", 50)
                print(f"  {s['name']}: {trust_state(trust)} ({trust}/100)")

    else:
        print("\n(Hints and legend are hidden in hard mode.)")
    wait_for_space()
...
def show_quests():
    ...
    for quest in game_state["quests"]:
        ...
        if (
            quest.startswith("\033[9mInvestigate the disappearance of the Bishop.")
...
def show_map():
    ...
    # --- Scrying Lens overrides fog of war and NPC visibility ---
    has_scrying_lens = "Scrying Lens" in game_state.get("inventory", [])
    fog_of_war = game_state.get("fog_of_war", False)
    show_npcs = game_state.get("can_see_npcs", False)
    if has_scrying_lens:
        fog_of_war = False
        show_npcs = True

    render_map(show_npcs, fog_of_war=fog_of_war)
...
def deduction_tree_for_suspect(suspect_name):
    ...
    # Show trust state in easy mode
    if easy_mode:
        trust = suspect.get("trust", 50)
        print(f"├─ Trust Level: {trust_state(trust)} ({trust}/100)")
        if suspect.get("blackmailed"):
            print("├─ Blackmailed: Yes")
        if suspect.get("favor_pending"):
            print("├─ Owes Favor: Yes")
        if trust >= 60:
            print(f"│   ├─ {YELLOW}High trust: This suspect is likely to tell the truth and may help challenge other alibis.{RESET}")
        elif trust <= 39:
            print(f"│   ├─ {RED}Low trust: This suspect may lie or mislead you.{RESET}")
    ...
    if easy_mode:
        print("└─ (Gather more evidence for a stronger deduction. Trust levels affect truthfulness and cooperation.)\n")
    else:
        print("└─ (Deduction details are limited in hard mode.)\n")
    wait_for_space()
...
def show_inventory():
    ...
    print(f"[{len(inventory)+1}] Return")

    choice = input("\nSelect an item to inspect/use, or RETURN to action menu: ").strip().lower()

    # Return to previous menu
    if choice == "" or (choice.isdigit() and int(choice) == len(inventory) + 1):
        if previous_menu_function:
            previous_menu_function()
        else:
            describe_room()
    ...
    if choice.isdigit():
        ...
        if 0 <= idx < len(inventory):
            item = inventory[idx]
            artifact = next((a for a in artifact_pool if a["name"] == item), None)
            potion = next((p for p in potion_pool if p["name"] == item), None)
            # Passive-use artifacts (not directly usable)
            passive_artifacts = {
                "Abraxas Amulet", "Elder Sign", "Scrying Lens",
                "First Fragment of the Eibon Manuscript", "Second Fragment of the Eibon Manuscript", "Third Fragment of the Eibon Manuscript"
            }
            is_usable = (
                potion is not None or
                (artifact and artifact.get("effect") in [
                    "sanity", "faith", "strength", "stamina", "perception", "agility", "clue", "repel", "shortcut",
                    "warn", "ritual", "quest", "hint", "secret", "random", "combat", "unlock", "puzzle"
                ])
                or (artifact and artifact["name"] == "Book of Eibon")
            )
            while True:
                print(f"\n{item}:")
                print("[1] Inspect")
                if is_usable and item not in passive_artifacts:
                    print("[2] Use")
                print("[3] Return")
...
— Commissioner of Police of the Metropolis
                        ...
                        # Special: Book of Eibon dynamic description
                        if artifact["name"] == "Book of Eibon":
                            required_pages = {
                                "First Fragment of the Eibon Manuscript",
                                "Second Fragment of the Eibon Manuscript",
                                "Third Fragment of the Eibon Manuscript"
                            ...
                            if game_state.get("eibon_complete") or required_pages.issubset(player_pages):
                                delay_print("Book of Eibon (Complete): A legendary tome of forbidden knowledge, now whole. Its pages pulse with cosmic power.")
                            else:
                                delay_print("Book of Eibon (Incomplete): A legendary tome of forbidden knowledge, its pages scattered and power incomplete.")
                            wait_for_space()
                            break
                        else:
                            delay_print(f"{artifact['name']}: {artifact['desc']}")
                            wait_for_space()
                    elif potion:
                        delay_print(f"{potion['name']}: {potion['desc']}")
                        wait_for_space()
                    else:
                        delay_print(f"You inspect the {item}, but it does not demonstrate any special properties for immediate use.")
                        wait_for_space()
                elif sub_choice == "2" and is_usable and item not in passive_artifacts:
                    # Use
                    if artifact:
                        # --- Bishop’s Signet Ring: Deduction Puzzle ---
                        if artifact["name"] == "Bishop’s Signet Ring":
                            clear()
                            delay_print(f"{YELLOW}You slip on the Bishop’s Signet Ring. Patterns in the suspects’ alibis seem to leap out at you...{RESET}")
                            # Build a grid of suspects, times, and locations
                            timeline = build_timeline_data()
                            times = sorted(timeline.keys())
                            locations = sorted({loc for t in timeline.values() for loc in t})
                            print("\nSuspect Alibi Grid:")
                            print("Time      | " + " | ".join(f"{loc[:12]:<12}" for loc in locations))
                            print("-" * (11 + len(locations) * 15))
                            for time in times:
                                row = f"{time:<9} | "
                                row += " | ".join(
                                    ", ".join(timeline[time].get(loc, []))[:12].ljust(12)
                                    if timeline[time].get(loc) else " " * 12
                                    for loc in locations
                                )
                                print(row)
                            # Find contradictions: same time, same room, >1 suspect
                            contradictions = []
                            for time in times:
                                for loc in locations:
                                    suspects_here = timeline[time].get(loc, [])
                                    if len(suspects_here) > 1:
                                        contradictions.append((time, loc, suspects_here))
                            if contradictions:
                                print(f"\n{ORANGE}Contradiction(s) detected! Multiple suspects claim to be in the same room at the same time:{RESET}")
                                for time, loc, suspects in contradictions:
                                    print(f"  - At {time} in {loc}: {', '.join(suspects)}")
                                delay_print(f"{CYAN}You confront the suspects about this contradiction. One of them falters, and you sense a lie!{RESET}")
                                # Reward: Increase trust with all but one, decrease trust with one random liar
                                liar = random.choice(contradictions[0][2])
                                for s in game_state["suspects"]:
                                    if s["name"] in contradictions[0][2]:
                                        if s["name"] == liar:
                                            adjust_trust(s, -10)
                                            delay_print(f"{RED}{s['name']} is caught in a lie! Trust decreases.{RESET}")
                                        else:
                                            adjust_trust(s, +5)
                                            delay_print(f"{YELLOW}{s['name']} seems relieved to be believed. Trust increases.{RESET}")
                                game_state["journal"].append("Used the Bishop’s Signet Ring to spot a contradiction in alibis.")
                            else:
                                delay_print(f"{CYAN}No contradictions found. The suspects’ alibis appear consistent—for now.{RESET}")
                            wait_for_space()
                            break
                    if artifact:
                        # --- Scrying Lens special use ---
                        if artifact["name"] == "Scrying Lens":
                            if not game_state.get("can_see_npcs", False) or game_state.get("fog_of_war", False):
                                game_state["can_see_npcs"] = True
                                game_state["fog_of_war"] = False
                                delay_print(f"{CYAN}You peer through the Scrying Lens. The fog lifts from your mind, and you see the true layout of the manor and the locations of its inhabitants!{RESET}")
                            else:
                                delay_print("You peer through the Scrying Lens, but you already see all that it can reveal.")
                            wait_for_space()
                            break
                        # --- Book of Eibon special use ---
                        if artifact["name"] == "Book of Eibon":
                            required_pages = {
                                "First Fragment of the Eibon Manuscript",
                                "Second Fragment of the Eibon Manuscript",
                            ...
                            player_pages = set(item for item in game_state["inventory"] if "Fragment of the Eibon Manuscript" in item)
                            if game_state.get("eibon_complete") or required_pages.issubset(player_pages):
                                # Merge pages if not already merged
                                if not game_state.get("eibon_complete"):
                                    merged = False
                                    for page in required_pages:
                                        if page in game_state["inventory"]:
                                            game_state["inventory"].remove(page)
                                            merged = True
                                    if merged:
                                        delay_print(f"{YELLOW}The manuscript fragments merge into the Book of Eibon, which is now complete!{RESET}")
                                    game_state["eibon_complete"] = True
                                use_book_of_eibon()
                                wait_for_space()
                                break
                            else:
                                delay_print("The Book of Eibon is incomplete. You must find all three manuscript fragments before you can use its power.")
                                wait_for_space()
                                break
                        # Track usage with a 'used' flag on the artifact (by name in game_state)
                        if "used_artifacts" not in game_state:
                            game_state["used_artifacts"] = set()
                        if artifact["name"] in one_use_artifacts:
                            if artifact["name"] in game_state["used_artifacts"]:
                                delay_print(one_use_artifacts[artifact["name"]])
                            else:
                                amt = artifact.get("amount", 0)
                                effect = artifact.get("effect")
                                if effect in game_state:
                                    if isinstance(amt, str) and amt.startswith("+"):
                                        amt = int(amt)
                                    game_state[effect] = min(18, game_state[effect] + int(amt))
                                    delay_print(f"You use the {artifact['name']}. {artifact['desc']} (+{amt} {effect.capitalize()})")
                                elif artifact["name"] == "Brass Spyglass":
                                    # Directional clue logic
                                    current_pos = game_state["position"]
                                    min_dist = 999
                                    nearest = None
                                    for pos, clue in clue_locations.items():
                                        if clue not in game_state["clues"]:
                                            ...
                                            if dist < min_dist:
                                                ...
                                                nearest = pos
                                    if nearest:
                                        dx = nearest[0] - current_pos[0]
                                        dy = nearest[1] - current_pos[1]
                                        direction = []
                                        ...
                                        if direction:
                                            delay_print(f"You peer through the spyglass. A faint shimmer points to the {' and '.join(direction)}.")
                                        else:
                                            delay_print("You peer through the spyglass, but see nothing new.")
                                    else:
                                        delay_print("You peer through the spyglass, but see nothing new.")

                                game_state["used_artifacts"].add(artifact["name"])
                            wait_for_space()
                        else:
                            # Permanent artifact: always present, only works once, then "nothing happens"
                            if "used_artifacts" not in game_state:
                                game_state["used_artifacts"] = set()
                            if artifact["name"] in game_state["used_artifacts"]:
                                delay_print("Nothing happens.")
                            else:
                                amt = artifact.get("amount", 0)
                                effect = artifact.get("effect")
                                if effect in game_state:
                                    ...
                                    delay_print(f"You use the {artifact['name']}. {artifact['desc']}")
                                game_state["used_artifacts"].add(artifact["name"])
                            wait_for_space()
                    elif potion:
                        # Potions: one-use, but always remain in inventory
                        if "used_artifacts" not in game_state:
                            game_state["used_artifacts"] = set()
                        if potion["name"] in game_state["used_artifacts"]:
                            delay_print("The vial is empty.")
                        else:
                            ...
                            game_state["used_artifacts"].add(potion["name"])
                        wait_for_space()
                    break
                elif sub_choice == "3" or (sub_choice == "2" and (not is_usable or item in passive_artifacts)):
                    break
                else:
        ...
        if 0 <= idx < len(usable):
            ...
            if artifact:
                ...
                if "side_effects" in artifact:
                    ...
                    # Always treat as a list of tuples, even if single tuple
                ...
                if "side_effect" in artifact:
                    stat, amt = artifact["side_effect"]
                    game_state[stat] = max(0, min(18, game_state[stat] + amt))
                    delay_print(f"Side effect: {stat.capitalize()} {'+' if amt > 0 else ''}{amt}")
                # Remove after use (permanent effect -- REMOVED)
...
def print_eibon_narrative():
    """
    Print the full narrative for reading the completed Book of Eibon.
    """
    delay_print(
        f"""{WHITE}(Chapter XIV — Of the City Beneath the Sea and the Wheel of the Void
(attributed to Abdul Alhazred, in an untranslated script bound between vellum leaves, recovered from a sealed reliquary in Harran. Translated from the Hyperborean by Erastes Kalon, 1627){RESET}"""
    )
    print()  # 1) carriage return after 1627)...

    delay_print(
        f"""{WHITE}It was in the Third Cycle of my wanderings that I beheld the Gate of Flutes. It is a wound in space, not a door. Through it I heard the piping—first like children's songs, then like molten obsidian scraping a bronze sky. From it pours the black wind that heralds He who Blinds with Music. And upon the crest of that wind floats the broken syllables of Azathoth's name, which unravels minds and calendars alike...{RESET}"""
    )
    print()  # 2) carriage return after calendars alike...

    delay_print(
        f"""{WHITE}The city of R'lyeh was wrought not by hands, but by calculation. Angles beyond Euclid. Its streets are not aligned to earth, nor to sun, but to the geometry of the Others. Its towers are cast in stone that grows. Its gates open inward to places that never were. There is no horizon there, only curve. All compasses fail. All thought bleeds. I entered once in dream, and was chased by something that had my name but wore no face...{RESET}"""
    )
    print()  # 3) carriage return after wore no face...

    delay_print(
        f"""{WHITE}Beyond this lies the Court of the Blind Idiot, whose dreams shape the chaos of void and flame. To summon His eye is to cast one's soul into that swirling furnace—but to understand Him, to name His hungers, is to master the pulse of a thousand broken worlds. The way is paved by symbols: the sigil of Abraxas, engraved not on the hand but on the mind. Wielded rightly, it shields the bearer from the clawed ones who prowl beneath thought...{RESET}"""
    )
    print()
    delay_print(
        f"{WHITE}Know this: the Elder Ones are not gods. They do not crave worship. They neither punish nor reward. They hunger, and they remember. To name them is to invoke the memory of a cosmos before men, before stars, before hope. And yet, to know them is power. Not power in the manner of kings or priests, but the power to step outside of time, and be devoured last.{RESET}"
    )
    print()
    # Marginal note: italics ONCE at start, RESET ONCE at end
    delay_print(f"{GREY}\033[3mA second marginal note, penned in a different hand—perhaps Bishop Greaves, the ink fresher, more deliberate:\n")
    delay_print("They gather beyond my study’s pane at night, pressing soft fingers to the frost. The Manor watches too—it hums. "
                "I have found the Abraxas amulet locked within the hearthbrick of the old music room. It was never lost, only kept by the architect in secret.\n")
    delay_print("THEY DO NOT TOUCH ME WHEN I WEAR IT. Even the cellar remains still—no breathing from the cracks.\n")
    delay_print("There is talk in dreams of a man who once bore command in a sand-blasted land. He comes not as a rival, but a herald, hollowed and forgotten. "
                "Thoth-Amon, they call him. A pawn once—perhaps again. Even in ruin, he walks the margins of the veil.\n")
    print()
    delay_print("The dream returned. The angle in the Manor's west wing now seems...wrong. The chandelier moved on its own, not in sway but in orbit. "
                "There was a pulse under the floorboards—like breath. I touched the Abraxas amulet and it went still.\n")
    delay_print("The city described above—R’lyeh—seems to match the old star charts hidden beneath the chapel's altar. "
                "Perhaps even the devouring pit beneath the well is a siphon... not into earth, but into there.\n")
    delay_print("The music has started faintly—thin reedy notes. At night I see things pass the mirror. I am not alone in the Manor. Not anymore.\n")
    delay_print("I continue the rites. May the angels cover their eyes. —† Greaves \033[0m{RESET}")

def use_book_of_eibon():
    """Allows the player to read or evoke the Book of Eibon if it's in their inventory."""
    clear()
    print(f"{YELLOW}You open the Book of Eibon...{RESET}\n")
    # Always treat as complete if eibon_complete flag is set
    if game_state.get("eibon_complete"):
        print(f"{CYAN}The Book is complete. Its forbidden pages pulse with cosmic power.{RESET}")
        print(f"\n[R] Read the Book of Eibon")
        print(f"[E] Evoke the Book's power...")
        print("[Q] Close the book")
        while True:
            choice = input("> ").strip().lower()
            if choice == "r":
                clear()
                delay_print(f"{ORANGE}You begin to read the completed Book of Eibon...{RESET}")
                print_eibon_narrative()
                wait_for_space()
                clear()
                print(f"\n[R] Read again\n[E] Evoke the Book's power\n[Q] Close the book")
            elif choice == "e":
                delay_print(f"{RED}You begin the forbidden ritual, invoking the Book's power...{RESET}")
                wait_for_space()
                evoke_book_of_eibon()
                return
            elif choice == "q":
                delay_print("You close the Book of Eibon.")
                wait_for_space()
                if previous_menu_function:
                    previous_menu_function()
                else:
                ...
                print("Choose [R], [E], or [Q].")
    else:
        # If somehow called before assembly, show incomplete menu
        print(f"{ORANGE}The Book is incomplete. Its pages are scattered and the secrets within are fragmented.{RESET}")
        print(f"\n[R] Read the incomplete Book")
        print("[Q] Close the book")
        while True:
            choice = input("> ").strip().lower()
            if choice == "r":
                clear()
                delay_print(f"{ORANGE}You read the tattered, incomplete Book of Eibon...{RESET}")
                delay_print(f"{GREY}(...Abraxas hnahr'luhnah ephaiah mgepah'ehye geb...){RESET}")
                wait_for_space()
                clear()
                print(f"\n[R] Read again\n[Q] Close the book")
            elif choice == "q":
...
def evoke_book_of_eibon():
    """Triggers the portal sequence as if the player is ready to face the final boss."""
    magical_pinwheel_animation()
    delay_print(f"{RED}The Book of Eibon trembles in your hands...{RESET}")
    # wait_for_space()
    final_boss_sequence()
...
def magical_pinwheel_animation():
    frames = [
        r"""
    ...
    for i in range(3):  # Repeat the swirl a few times
        for frame in frames:
            os.system('cls' if os.name == 'nt' else 'clear')
            print(frame)
            time.sleep(0.13)
    os.system('cls' if os.name == 'nt' else 'clear')
    print("\n" * 3 + " " * 10 + "\033[95mTHE MIND REELS...\033[0m\n" * 1)
    time.sleep(1.2)
...
def skill_check_combat(enemy_name, enemy_difficulty=None, stat=None):
    ...
    while player_stamina > 0 and enemy_stamina > 0:
        ...
        print(f"Your Stamina: {player_stamina} | {enemy_name}'s Stamina: {enemy_stamina}")
        if transformed:
            print(f"Your Sanity: {game_state['sanity']}")
        if game_state["sanity"] < 4:
            print(RED + "WARNING: Your sanity is dangerously low!" + RESET)
        if player_agility < 4:
            print(RED + "WARNING: Your agility is dangerously low!" + RESET)
        if player_strength < 4:
            print(RED + "WARNING: Your strength is dangerously low!" + RESET)
        if player_stamina < 4:
            print(RED + "WARNING: Your stamina is dangerously low!" + RESET)   

        while True:
            print("[A] Attack  [U] Use item  [F] Flee  [S] Show stats (or enter a global command)")
            action = input("> ").strip().lower()
            if action in ['a', 'u', 'f', 's', '']:
                break
            else:
                handle_input(action, lambda: None)
        ...
        if action == "u":
            ...
            if transformed:
                ...
                print(f"Your escape roll: {game_state['agility']} + die roll = {YELLOW}{player_agility_score}{RESET}")
                print(f"{enemy_name}'s pursuit roll: {enemy_agility} + die roll = {YELLOW}{enemy_agility_score}{RESET}")
                if player_agility_score >= enemy_agility_score:
                    print(f"{YELLOW}You slip away before your opponent can react!{RESET}")
                    game_state["stamina"] = player_stamina
                    wait_for_space()
                    describe_room()
                    return
                else:
                    print(RED + "You try to flee, but your opponent catches you with a parting blow! (Stamina -2, Score -10)" + RESET)
        ...
        print(f"Your combat score: {player_base} + die roll + agility mod = {YELLOW}{player_roll}{RESET}")
        print(f"Enemy combat score: {enemy_base} + die roll + agility mod = {YELLOW}{enemy_roll}{RESET}")
        ...
        if player_roll >= enemy_roll:
            ...
            if transformed and "Abraxas Amulet" in game_state["inventory"]:
                delay_print(f"{ORANGE}The Abraxas Amulet glows! The monstrosity cannot harm you while you possess it.{RESET}")
                damage = 0
            ...
            if elder_god_twist:
                ...
                print("You reel in terror as the truth is revealed: this was no mere mortal, but a servant of some higher unspeakable evil from some other upside down plane of existence!")
                game_state["sanity"] = max(0, game_state["sanity"] - 1)
                sanity_lost = 2
            else:
                if transformed:
                    print(ORANGE + random.choice([
                        f"The monstrosity tries to rend the flesh from your bones!",
                        f"With a guttural roar, the creature's claws slash at your throat, spraying blood across the walls.",
                        f"The abomination lunges, jaws distending to snap at your face.",
                        f"Tentacles burst from its torso, writhing and seeking to strangle you.",
                    ...
                    print(f"You lose {damage} stamina.")
                    print("\n" + random.choice([
                        f"As {enemy_name} strikes you, their flesh ripples and splits, revealing writhing tentacles and lidless, unblinking eyes.",
                        f"{enemy_name}'s mouth distends impossibly wide, emitting a chorus of whispers in a language not meant for human ears.",
                        f"Your attacker’s skin sloughs away, replaced by a mass of chitinous plates and twitching, alien appendages.",
    ...
    if suspect and transformed and player_stamina > 0 and enemy_stamina <= 0:
        ...
        if not used_webley and not used_amulet:
            print(f"{RED}Your attacks seem to do nothing! The monstrosity shrugs off mortal wounds and rises again, unkillable by ordinary force!{RESET}")
...
def handle_input(user_input, return_function):
    ...
    if user_input.strip().lower() == "superman":
        for stat in ["faith", "sanity", "perception", "strength", "stamina", "agility", "comeliness", "health"]:
            game_state[stat] = 18
        print(f"{YELLOW}MAXSTATS MODE: All player stats set to 18!{RESET}")
        wait_for_space()
        if callable(return_function):
            return_function()
        return
    ...
    if user_input in valid_moves:
        ...
        show_help()
    elif user_input == "gametime":
        elapsed = int((time.time() - game_state.get("start_time", time.time())) // 60)
        print(f"Total gametime: {elapsed} minutes")
        wait_for_space()
        if callable(return_function):
            return_function()
        return    
    elif user_input == "look" or user_input == "l" or user_input == "L":
        look_around()
        return_function()
        ...
        print("Available commands: help, look, save, load, quit, back, menu, title, inventory, journal, quests, stats, map, flow, score, fight, n/s/e/w ")
...
def show_help():
    ...
    print("- gametime: Show total game time in minutes")
    ...
    print("- n/s/e/w: Move in that direction from any screen")
    wait_for_space("Press SPACE to continue.")
...
def title_screen():
    ...
    clear()
    print(YELLOW +r"""
 ____   ___  ____    _____ __ __   ____  _____ ____  ___   ____  
|    \ /  _]|    \  / ___/|  |  | /    |/ ___/|    |/   \ |    \ 
|  o  )  [_ |  D  )(   \_ |  |  ||  o  (   \_  |  ||     ||  _  |
|   _/    _]|    /  \__  ||  |  ||     |\__  | |  ||  O  ||  |  |
|  | |   [_ |    \  /  \ ||  :  ||  _  |/  \ | |  ||     ||  |  |
|  | |     ||  .  \ \    ||     ||  |  |\    | |  ||     ||  |  |
|__| |_____||__|\_|  \___| \__,_||__|__| \___||____|\___/ |__|__|
    ...
    if user_input == "1":
        ...
        load_game()
    elif user_input == "7" or user_input == "quit" or user_input == "qq" or user_input == "exit":
        print()
        print(f"{YELLOW}If you enjoyed the game, please consider rating it on itch.io!{RESET}")
...
def instructions():
    ...
    - Gather enough clues and interrogate enough suspects
    - Use deduction to accuse the correct culprit
    - Avoid unnecessary violence for bonus achievements
    - Solve the mystery before your sanity or health runs out!

    Global Commands (usable at any prompt):

    - help: Show help message
    - save: Save your game
    - load: Load a previous save
    - quit: Quit the game
    - back/menu: Return to the previous menu
    - title: Return to the title screen
    - inventory [i]: Show your inventory
    - journal [j]: Show your journal entries
    - quests/mystery: Show active mysteries
    - stats: Show character stats
    - map [m]: Show the current map
    - flow: Show a simplified map flow of key rooms
    - score: Show your current score and scoring breakdown
    - fight: Start a combat encounter with any suspect (debug)
    - N/S/E/W: Move in that direction from any screen
    """
    ) 
    wait_for_space()
    clear()
    print(
        """
    How to Play Persuasion: Victorian Roguelike Investigation RPG
   
    - Search rooms to find clues, artifacts, and potions. Every room may hide something useful or dangerous.
    - Interrogate suspects you encounter. Listen to their alibis, question their motives, and look for contradictions. 
    - Each suspect has a tolerance—too many questions may anger them and trigger hostility or combat.
    - Use clues and deduction trees in your journal to compare suspects, spot contradictions, and build your case.
    - Combat is risky and triggered if a suspect becomes hostile. 
    - During combat, choose to attack, use an item, flee, or check stats. Dice rolls and your stats determine outcomes.
    - Fleeing is possible in combat (press 'F'), but may cost score or sanity. 
    - Avoid rousing suspects into hostility by varying your questions, watching their tolerance, and using persuasion or faith to calm them.
    - Once you have enough clues and have interrogated enough suspects, you may attempt to solve the case by making an accusation.
    - Violence is discouraged; solving the case through deduction is rewarded with achievements and a better ending.
    - Use your journal, map, and deduction tools often. Every clue and contradiction matters!

    Interrogating Suspects
    - If a suspect is present, [6] allows you to interrogate them.
    - Interrogation reveals motives, alibis, relationships, and other evidence.
    - Each suspect has a tolerance—too many questions may anger them and trigger hostility or combat.
    - Vary your questions, observe body language, and use persuasion (if available) to reset tolerance.
    - Present clues as evidence to get reactions.
    
    Clues
    - Found by searching rooms, inspecting artifacts, or interrogating suspects.
    - Each clue links to a motive and helps narrow down the culprit.
    
    Deduction Trees
    - In the journal, use [D] to view a suspect’s deduction tree: motive, alibi, corroborators, means, and clues.
    - Use deduction trees and other tools to compare suspects, spot contradictions, and build your case.
           
    Combat
    - Triggered if a suspect becomes hostile or you attack.
    - At each round, choose to attack, use an item, flee, or check stats. Dice rolls and stats determine outcomes.
    - Use [F] in combat to attempt escape.

     Tip: Trust your wits, examine each room, gather evidence, avoid violence and let deduction be your guide. 
...
.::: . @ @         :@ @@.  .     --   @ @   -*: . 
 ...
 . ..      ..  %@* -#@++%%%@*.... .  .::..::...  .
                  ...
                  He stirs beyond time, mindless and immense.
                 The heart of all existence, pulsing with madness.
                     You saw too much... and now you forget.
    """

    clear()  # Clear the screen
    for line in ascii_art.splitlines():
        print(line)
    input("\n\nPress Enter to confront Azathoth (NO CHOICE NOW!)...")
...
def reset_game_state():
    ...
    game_state["position"] = (0, 0)
    game_state["score"] = 0
    game_state["score_log"] = []
    game_state["achievements"] = set()
    game_state["suspects_interrogated"] = set()
    game_state["breadcrumbs"] = set()
    game_state["sanity_zero_penalized"] = False
    game_state["room_flavor_used"] = {}
    game_state["supernatural_rooms"] = []
    game_state["walls"] = set()
    game_state["locked_doors"] = set()
    game_state["passages"] = {}
    game_state["artifact_locations"] = {}
...
def add_random_clue():
    ...
    # Artifact/chest logic
    artifact = game_state.get("artifact_locations", {}).get(room)
    if artifact and artifact["name"] not in game_state["inventory"]:
        if artifact.get("type") == "chest":
            if artifact.get("opened"):
                delay_print("You see an empty chest here. Its contents have already been claimed.")
            else:
                print(f"\nYou spot a chest here: {artifact['desc']}")
                has_key = any(key in game_state["inventory"] for key in ["Strange Key", "Bent Key", "Silver Ornate Key"])
                if has_key:
                    print(f"{ORANGE}[U] Use a key to open the chest{RESET}")
                else:
                ...
                if choice == "u" and has_key:
                    ...
                    reward = artifact.get("content", "nothing")
                    print(f"You unlock the chest and find: {reward}!")
                    game_state["inventory"].append(reward)
                    game_state["journal"].append(f"Opened chest in {room}: {reward}")
                    game_state["score"] += 15
                    found_something = True
                    # Achievement check
                    all_chests = [a for a in game_state.get("artifact_locations", {}).values() if a.get("type") == "chest"]
                    if all(a.get("opened") for a in all_chests):
                        if "Opened every chest" not in game_state["achievements"]:
                            game_state["achievements"].add("Opened every chest")
                            game_state["score"] += ACHIEVEMENT_SCORE
                            print(f"{CYAN}Achievement unlocked: {ACHIEVEMENTS['Opened every chest']} (+{ACHIEVEMENT_SCORE} points){RESET}")
    ...
    if not found_something:
        ...
        wait_for_space("Press SPACE to continue.")     # input("Press Enter to continue.")
    # room = game_state["location"]
    # found_something = False
...
def show_accusation_breakdown(accused, correct_culprit, clue_motives, player_clues):
    ...
    # If the accusation was false, reveal the true culprit and their logic
    if accused["name"] != correct_culprit["name"]:
        print(f"\n{YELLOW}The true culprit was: {correct_culprit['name']}{RESET}")
        print(f"- Motive: {YELLOW}{culprit_motive}{RESET} {GREY}{correct_culprit.get('motive_long', '')}{RESET}")
        print(f"- Means: {YELLOW}{correct_culprit.get('means', 'Unknown')}{RESET}")
        culprit_alibi_text = correct_culprit.get('alibi', {}).get('text', 'Unknown')
        print(f"- Alibi: {YELLOW}{culprit_alibi_text}{RESET}")
        # Show clues that actually point to the culprit
        culprit_matching_clues = [clue for clue in player_clues if clue_motives.get(clue) == culprit_motive]
        if culprit_matching_clues:
            print(f"- Clues linking to {correct_culprit['name']}:")
            for clue in culprit_matching_clues:
                flavor = clue_flavor_explanations.get(clue, {}).get(culprit_motive)
                if flavor:
                    print(f"    {YELLOW}{clue}{RESET} {GREY}— {flavor}{RESET}")
                else:
                    print(f"    {YELLOW}{clue}{RESET}")
        else:
            print(f"- No clues directly link {correct_culprit['name']} to the crime.")
...
def final_boss_sequence():
    ...
    delay_print(
        ...
        'You puny mortals, so easily manipulated by greed, lust, jealousy... all part of my plan.\n'
    ...
    while player_stamina > 1 and azathoth_stamina > 0:
        clear()
        print(f"{RED}Azathoth looms before you, a storm of chaos and hunger!{RESET}")
        print(f"Your Stamina: {player_stamina} | Azathoth's Power: {azathoth_stamina}")
        print("\nWhat will you do?")
        print("[A] Attack")
        if has_amulet:
            print(f"{ORANGE}[U] Use Abraxas Amulet{RESET} or another item")
        else:
            print("[U] Use an item")
        ...
        if action == "u":
            if has_amulet:
                print(f"{ORANGE}You may use the Abraxas Amulet or another item from your inventory.{RESET}")
                print("[1] Use Abraxas Amulet")
                print("[2] Use another item")
                choice = input("> ").strip()
                if choice == "1":
                    ...
                    # Let player use any item (gun, etc.)
    ...
    if azathoth_stamina == 0:
        ...
        delay_print(
            "\"You will live, little wretch. I have need of you yet. There are other doors to open, other worlds to shatter. Rest now, and await my call...\""
        )
        show_epilogue_awaken_in_manor()
        delay_print(f"Final Score: {game_state['score']}")
        wait_for_space()
...
def show_score():
    ...
    # --- Trust Achievements and Rewards ---
    trust_scores = [s.get("trust", 50) for s in game_state["suspects"]]
    loyal_suspects = [s for s in game_state["suspects"] if s.get("trust", 0) >= 80]
    if loyal_suspects:
        for s in loyal_suspects:
            trophy = f"Loyalty Trophy: {s['name']} trusted you deeply."
            if trophy not in game_state["achievements"]:
                game_state["achievements"].add(trophy)
                game_state["score"] += 10
                print(f"{CYAN}Achievement unlocked: {trophy} (+10 points){RESET}")
    # All suspects loyal
    if all(t >= 80 for t in trust_scores) and trust_scores:
        trophy = "All suspects ended Loyal or Trusting"
        if trophy not in game_state["achievements"]:
            game_state["achievements"].add(trophy)
            game_state["score"] += 20
            print(f"{CYAN}Achievement unlocked: {trophy} (+20 points){RESET}")

    # Confession reward
    confessed = [s for s in game_state["suspects"] if s.get("confession", "").lower().startswith("fine, i admit it") or "confess" in s.get("confession", "").lower()]
    if confessed:
        for s in confessed:
            trophy = f"Confession Trophy: {s['name']} confessed to you."
            if trophy not in game_state["achievements"]:
                game_state["achievements"].add(trophy)
                game_state["score"] += 15
                print(f"{CYAN}Achievement unlocked: {trophy} (+15 points){RESET}")
    ...
    print("Suspect trust 80 or higher: +10 each")
    print("All suspects Loyal/Trusting: +20")
    print("Suspect confession: +15")
    elapsed = int((time.time() - game_state.get("start_time", time.time())) // 60)
    print(f"\nTotal turns played: {game_state.get('turns', 0)}")
    print(f"Total gametime: {elapsed} minutes")
...
def case_resolution():
    ...
    if accused and accused.get("is_murderer"):
        ...
        wait_for_space()
        if accused and accused.get("is_murderer"):
            game_state["enough_evidence_shown"] = True
...
def describe_room():
    ...
    main_quest_stricken = any(
        q.startswith("\033[9mInvestigate the disappearance of the Bishop.")
    ...
    # Only trigger quest completion and journal update ONCE, but do NOT start the boss fight
        if (
            "Assemble the Book of Eibon and open the portal to the Elder Ones' realm." not in [
                q.replace("\033[9m", "").replace("\033[0m", "") for q in game_state.get("quests", [])
            ]
            and "Assemble the Book of Eibon and open the portal to the Elder Ones' realm." not in game_state["journal"]
        ):
            delay_print(f"{YELLOW}You have assembled all three missing pages of the Book of Eibon!{RESET}")
            delay_print("The Book pulses with power. You feel the urge to use it...")
    ...
    if (
        ...
        and not game_state.get("enough_evidence_shown", False)
    ):
    ...
    print("[L] Look / Search the room")
    # print("[2] Move to another location")
    print("[M] Show map")
    print("[J] Check journal")
    ...
    if suspects_here:
        ...
        print(f"{ORANGE}[Q] Question / Interrogate suspect ({suspect_names}){RESET}")
    ...
    if (
        ...
        print(f"{ORANGE}[SOLVE] ATTEMPT CASE RESOLUTION{RESET}")
        case_resolution_available = True
    else:
        case_resolution_available = False
    
    print("[H] Lists global commands")
    print("[qq] Quit to title screen")
    print("[ss] Save game")
    ...
    if user_input in ["n", "s", "e", "w"]:
        ...
        describe_room()
        return
    elif user_input == "L" or user_input == "l" or user_input == "look" or user_input == "search":
        look_around()
        add_random_clue()
        game_state["turns"] += 1
        if game_state["turns"] % 15 == 0:
    ...
    #         describe_room()
        ...
        show_journal()
    elif user_input == "H" or user_input == "help" or user_input == "h":
        show_help()
    elif user_input == "I":
        show_inventory()
    elif user_input == "Q" or user_input == "q"and suspects_here:
        interrogate_suspect()
    elif user_input == "SOLVE" or user_input == "solve" and case_resolution_available:
        case_resolution()
    elif user_input == "ss" or user_input == "save":
        save_game()
        describe_room()
    elif user_input == "qq" or user_input == "quit":
        title_screen()
...
def adjust_trust(suspect, amount):
    old_trust = suspect.get("trust", 55)
    suspect["trust"] = max(0, min(100, old_trust + amount))
    if amount > 0:
        delay_print(f"{YELLOW}(Trust increased by +{amount}: {old_trust} → {suspect['trust']}){RESET}")
    elif amount < 0:
        delay_print(f"{RED}(Trust decreased by {amount}: {old_trust} → {suspect['trust']}){RESET}")
    # If amount == 0, no message
...
def interrogate_suspect():
    ...
    while True:
        ...
        for i, s in enumerate(suspects_here):
            trust_score = s.get('trust', 55)
            trust_word = trust_state(trust_score)
            print(f"[{i+1}] {s['name']} (Trust: {trust_word} [{trust_score}/100])")
        ...
        if user_input.isdigit():
            ...
            if 0 <= idx < len(suspects_here):
                ...
                response_count = 0
                if "presented_clues" not in suspect:
                    suspect["presented_clues"] = set()
                while True:
                    clear()
                    print("\nOptions:")
                    print("[1] Observe body language")
                    print("[2] Act kindly/honestly")
                    ...
                    print("[10] Ask about occult matters")
                    print("[11] Direct accusation")
                    if game_state["faith"] >= 14:
                        if not suspect.get("bless_option_shown", False):
                            print(f"{ORANGE}[12] Bless or rebuke the suspect (Faith){RESET}")
                            suspect["bless_option_shown"] = True
                        else:
                            print("[12] Bless or rebuke the suspect (Faith)")
                    if game_state.get("clues"):
                        if not suspect.get("present_clue_option_shown", False):
                            print(f"{ORANGE}[13] Present a clue as evidence{RESET}")
                            suspect["present_clue_option_shown"] = True
                        else:
                            print("[13] Present a clue as evidence")
                    if suspect.get("trust", 50) > 80 and suspect.get("trust_clue"):
                        if not suspect.get("trust_clue_option_shown", False):
                            print(f"{ORANGE}[14] Ask about hidden knowledge{RESET}")
                            suspect["trust_clue_option_shown"] = True
                        else:
                            print("[14] Ask about hidden knowledge")
                    if (
                        not game_state.get("persuasion_active", False)
                        and (game_state.get("comeliness", 0) >= 14 or game_state.get("perception", 0) >= 14)
                        and not suspect.get("persuade_attempted", False)
                        and suspect.get("trust", 50) > 50
                    ):
                        if not suspect.get("persuade_option_shown", False):
                            print(f"{ORANGE}[P] Attempt Persuasion{RESET}")
                            suspect["persuade_option_shown"] = True
                        else:
                            print("[P] Attempt Persuasion")
                    has_blackmail = suspect.get("blackmail") and suspect["blackmail"] in game_state.get("clues", [])
                    has_favor = suspect.get("favor_pending", False)
                    if has_blackmail or has_favor:
                        if not suspect.get("leverage_option_shown", False):
                            print(f"{ORANGE}[15] Use leverage (blackmail or quid pro quo (favor)){RESET}")
                            suspect["leverage_option_shown"] = True
                        else:
                            print("[15] Use leverage (blackmail or quid pro quo (favor))")
                    # --- Option 16: Return a personal item (favor) ---
                    if suspect.get("favor_item") and suspect["favor_item"] in game_state["inventory"] and suspect.get("favor_pending"):
                        # Show flavor hint if present
                        if suspect.get("favor_hint"):
                            print(suspect["favor_hint"])
                        print("[16] Return a personal item (favor)")
                    ...
                    def check_tolerance():
                        trust = suspect.get("trust", 50)
                        base_tolerance = game_state.get("suspect_tolerance", 5)
                        if trust >= 60:
                            suspect["tolerance"] = suspect.get("tolerance", base_tolerance) - 0.5
                        else:
                            suspect["tolerance"] = suspect.get("tolerance", base_tolerance) - 1
                        if suspect["tolerance"] <= 0:
                            if trust < 40:
                                delay_print(f"{suspect['name']} becomes hostile!")
                                skill_check_combat(suspect["name"])
                                if suspect not in game_state["suspects"]:
                                    describe_room()
                                    return True
                            else:
                                delay_print(f"{suspect['name']} is clearly annoyed, but remains civil due to your rapport.")
                                suspect["tolerance"] = 1
                        return False

                    if choice == "1":
                        # Only allow trust gain from observing body language once per suspect
                        if suspect.get("body_language_used", False):
                            delay_print("You observe their body language, but gain no new insight.")
                        else:
                        ...
                        process_trust_sequence(suspect, choice)
                        wait_for_space()
                        continue

                    elif choice == "2":
                        if suspect.get("kindness_used", False):
                            delay_print(f"{RED}{suspect['name']} eyes you warily. 'Enough with the flattery. What do you really want?' Trust decreases!{RESET}")
                            adjust_trust(suspect, -7)
                        else:
                            delay_print(f"{ORANGE}Your honesty/kindness earns {suspect['name']}'s trust.{RESET}")
                            adjust_trust(suspect, +5)
                            suspect["kindness_used"] = True
                        process_trust_sequence(suspect, choice)
                        wait_for_space()
                        continue
                        ...
                        if suspect.get("trust", 50) < 40:
                            if random.random() < 0.5:
                                fake_locations = [room for room in room_templates.keys() if room != alibi.get("location", "")]
                                fake_location = random.choice(fake_locations) if fake_locations else "Unknown"
                                fake_time = random_15min_time((21, 24))
                                fake_alibi = f"I was in the {fake_location} at {fake_time}."
                                delay_print(f"{RED}{suspect['name']} says: '{fake_alibi}' (This may be a lie!){RESET}")
                                alibi_text = fake_alibi
                            else:
                                delay_print(f"{RED}{suspect['name']} says: '{alibi_text}' (This may be a lie!){RESET}")
                        else:
                        ...
                        # Only allow challenging alibis of suspects who have been interrogated at least once
                        if suspect["name"] not in game_state.get("suspects_interrogated", set()):
                            delay_print(f"{RED}You must confront {suspect['name']} at least once before challenging their alibi or asking who they suspect.{RESET}")
                            wait_for_space()
                            continue

                        if suspect.get("trust", 55) <= 19:
                            innocent = [s for s in game_state["suspects"] if not s.get("is_murderer") and s["name"] != suspect["name"]]
                            if innocent:
                                framed = random.choice(innocent)
                                delay_print(f"{RED}{suspect['name']} says: 'If you ask me, {framed['name']} is definitely hiding something.' (This may be a lie!){RESET}")
                            else:
                                delay_print(f"{RED}{suspect['name']} refuses to cooperate and glares at you in silence.{RESET}")
                        else:
                        ...
                        if suspect.get("trust", 55) >= 60:
                            others = [s for s in game_state["suspects"] if s["name"] != suspect["name"]]
                            print(f"\n{YELLOW}Because {suspect['name']} trusts you, they may help challenge another suspect's alibi.{RESET}")
                            print("Challenge whose alibi?")
                            # Only show suspects who have been interrogated at least once
                            interrogated_names = set(game_state.get("suspects_interrogated", set()))
                            available_others = [other for other in others if other["name"] in interrogated_names]
                            for idx, other in enumerate(available_others, 1):
                                print(f"[{idx}] {other['name']}")
                            print(f"[{len(available_others)+1}] Cancel")
                            sub_choice = input("> ").strip()
                            if sub_choice.isdigit():
                                idx = int(sub_choice) - 1
                                if 0 <= idx < len(available_others):
                                    other = available_others[idx]
                                    alibi = other.get("alibi", {})
                                    time = alibi.get("time", "Unknown")
                                    loc = alibi.get("location", "Unknown")
                                    if suspect.get("alibi", {}).get("time") == time and suspect.get("alibi", {}).get("location") != loc:
                                        delay_print(f"{CYAN}{suspect['name']} says: 'I was at {suspect['alibi']['location']} at {time}, but {other['name']} claims to be elsewhere. Their story doesn't add up.'{RESET}")
                                    else:
                                        delay_print(f"{GREY}{suspect['name']} says: 'I can't contradict {other['name']}'s alibi directly, but something feels off.'{RESET}")
                                    # wait_for_space()
                        if random.random() < 0.2:
                            delay_print(f"{RED}{suspect['name']} is offended by your approach. Trust decreases.{RESET}")
                            adjust_trust(suspect, -7)
                        if check_tolerance():
                            return
                        wait_for_space()
                        ...
                        if suspect.get("bless_attempted", False):
                            delay_print(f"{RED}You have already attempted to bless or rebuke {suspect['name']}. They seem wary of further spiritual probing.{RESET}")
                        else:
                            suspect["bless_attempted"] = True
                            if random.randint(1, 18) <= game_state["faith"]:
                                delay_print(f"{YELLOW}Your blessing unsettles {suspect['name']}! You sense a shadow within...{RESET}")
                                adjust_trust(suspect, -10)
                                game_state["journal"].append(f"Blessed {suspect['name']}: Hidden evil sensed!")
                            else:
                                delay_print(f"{CYAN}{suspect['name']} seems moved by your blessing, but you sense no hidden evil.{RESET}")
                                adjust_trust(suspect, +3)
                                game_state["journal"].append(f"Blessed {suspect['name']}: No evil sensed.")
                        wait_for_space()
                        continue
                        ...
                        if game_state.get("clues"):
                            ...
                            if clue_choice.isdigit():
                                ...
                                if 0 <= clue_idx < len(game_state["clues"]):
                                    ...
                                    suspect["presented_clues"].add(presented_clue)
                                    clue_motive = game_state.get("clue_motives", {}).get(presented_clue)
                                    # --- Blackmail logic: presenting the blackmail clue ---
                                    if presented_clue == suspect.get("blackmail"):
                                        if suspect.get("is_murderer"):
                                            delay_print(f"{RED}{suspect['name']} stammers, sweat beading on their brow. 'W-where did you find that? I... I can't explain...'")
                                            if suspect.get("trust", 50) > 70:
                                                delay_print(f"{YELLOW}{suspect['name']} breaks down and confesses!{RESET}")
                                                game_state["forced_accused"] = suspect["name"]
                                                case_resolution()
                                                return
                                            else:
                                                delay_print(f"{suspect['name']} refuses to say more, but you sense their resolve is shaken.")
                                        elif suspect.get("believes_culprit"):
                                            delay_print(f"{CYAN}{suspect['name']} glances at the clue, then leans in: 'I know who did it. It was {suspect['believes_culprit']}.'{RESET}")
                                            adjust_trust(suspect, +10)
                                        else:
                                            delay_print(f"{ORANGE}{suspect['name']} is clearly rattled by the clue. Trust decreases!{RESET}")
                                            adjust_trust(suspect, -7)
                                        suspect["blackmailed"] = True
                                        wait_for_space()
                                        continue
                                    if suspect.get("trust", 50) <= 19:
                                        delay_print(f"{RED}{suspect['name']} says: 'That clue means nothing. You're barking up the wrong tree.' (This may be a lie!){RESET}")
                                    elif suspect.get("is_murderer") and suspect.get("trust", 50) > 60 and clue_motive == suspect.get("motive"):
                                        delay_print(f"{CYAN}{suspect['name']} stares at the clue, their composure breaks!{RESET}")
                                        delay_print(f"{YELLOW}{suspect['name']} confesses: 'It's true. I am responsible. The evidence is overwhelming.'{RESET}")
                                        game_state["forced_accused"] = suspect["name"]
                                        case_resolution()
                                        return
                                    elif clue_motive == suspect.get("motive") and not suspect.get("is_murderer"):
                                        delay_print(f"{CYAN}{suspect['name']} feels understood. Trust increases.{RESET}")
                                        adjust_trust(suspect, +7)
                                        delay_print(f"{suspect['name']} looks shaken. \"I... I don't know what to say about that.\"")
                                    elif clue_motive != suspect.get("motive"):
                                        delay_print(f"{RED}{suspect['name']} feels accused or misunderstood. Trust decreases.{RESET}")
                                        adjust_trust(suspect, -5)
                        ...
                        if suspect.get("trust", 50) > 80 and suspect.get("trust_clue"):
                            clue = suspect["trust_clue"]
                            delay_print(f"{CYAN}{suspect['name']} leans in and whispers, 'you may need this... ': '{clue}'{RESET}")
                            if clue not in game_state["clues"]:
                                game_state["clues"].append(clue)
                                game_state["journal"].append(f"CLUE FOUND (Trust): {clue}")
                            suspect["trust_clue"] = None
                        else:
                            delay_print(f"{suspect['name']} doesn't trust you enough to share anything deeper.")
                        wait_for_space()
                        continue

                    elif choice in ("persuade", "p"):
                        if game_state.get("comeliness", 0) < 14 and game_state.get("perception", 0) < 14:
                            delay_print(f"{RED}You lack the presence or insight to attempt persuasion.{RESET}")
                            wait_for_space()
                            continue
                        if suspect.get("persuade_attempted", False):
                            delay_print(f"{RED}You have already attempted persuasion with {suspect['name']}.{RESET}")
                            wait_for_space()
                            continue
                        if suspect.get("trust", 50) <= 50:
                            delay_print(f"{RED}{suspect['name']} does not trust you enough to be persuaded. (Trust must be above 50){RESET}")
                            wait_for_space()
                            continue
                        suspect["persuade_attempted"] = True
                        ...
                        comeliness = game_state.get("comeliness", 0)
                        perception = game_state.get("perception", 0)
                        avg_stat = (comeliness + perception) // 2
                        ...
                        fudge_roll = sum(random.choice([-1, 0, 1]) for _ in range(4))
                        skill_total = avg_stat + fudge_roll

                        delay_print(f"{CYAN}(Persuasion check: {avg_stat} + die roll {fudge_roll} = {skill_total}, Target: {target}){RESET}")

                        if skill_total >= target:
                        ...
                        has_favor = suspect.get("favor_pending", False)
                        if has_blackmail:
                            delay_print(f"{RED}You confront {suspect['name']} with their secret. They pale, realizing you have leverage.{RESET}")
                            adjust_trust(suspect, -10)
                            delay_print(f"{suspect['name']} grudgingly agrees to help, but you sense deep resentment.")
                            if suspect.get("trust_clue"):
                                delay_print(f"{CYAN}{suspect['name']} reveals: '{suspect['trust_clue']}'{RESET}")
                                game_state["clues"].append(suspect["trust_clue"])
                                suspect["trust_clue"] = None
                            suspect["blackmailed"] = True
                        elif has_favor:
                            delay_print(f"{CYAN}{suspect['name']} is grateful for your help. Trust increases!{RESET}")
                            adjust_trust(suspect, +10)
                            suspect["favor_pending"] = False
                        else:
                            delay_print(f"{suspect['name']} looks confused. You have no leverage over them.")
                        wait_for_space()
                        continue

                    elif choice == "16":
                        if suspect.get("favor_item") and suspect["favor_item"] in game_state["inventory"] and suspect.get("favor_pending"):
                            delay_print(f"{CYAN}You return {suspect['favor_item']} to {suspect['name']}. They are deeply grateful!{RESET}")
                            if suspect.get("believes_culprit"):
                                delay_print(f"{YELLOW}{suspect['name']} whispers: 'I trust you. The real culprit is {suspect['believes_culprit']}.'{RESET}")
                                adjust_trust(suspect, +15)
                            else:
                                delay_print(f"{suspect['name']} thanks you profusely. Trust increases!{RESET}")
                                adjust_trust(suspect, +10)
                            suspect["favor_pending"] = False
                            wait_for_space()
                            continue
                        else:
                            delay_print("You have nothing to return to this suspect.")
                            wait_for_space()
                            continue
...
def dream_flashback():
    ...
    clear()
    delay_print(f"\033[3mI am dreaming... or remembering. A shiver creeps down my spine as my mind conjures the weight of memory.{RESET}")
    print(f"\033[3mA chapel cloaked in shadow, the child's voice echoing like a song half-remembered. A locked door pulses in the back of my mind like a heartbeat.{RESET}")
    print(f"\033[3mWhispers echo backward through time and thought. I wake gasping, the image of a burning sigil fading from my vision.{RESET}")
    print()
    print("- note discovered on the floor near the chapel, signed by Bishop Alaric Greaves")
    wait_for_space()
    intro_scene()
...
def intro_scene():
    ...
    while True:
        ...
        # print("[2] Go to character creation")
        print("[2] Return to title screen")
...
def enter_manor():
    ...
    clear()
    delay_print("You step into the manor, the scent of mildew thick in your nostrils. ")
    print("The heavy door groans shut behind you, muffling the distant toll of church bells and sealing you in a hush broken only by the slow drip of unseen water. ")
    print("Shadows cling to the corners, and the faded grandeur of the entrance hall seems to watch your every move, as if the house itself is holding its breath, waiting for you to trespass further."
    )
    wait_for_space()
    show_detective_ascii()
    ...
    print("You take a deep breath, steeling yourself for what lies ahead. The manor is a labyrinth of rooms and corridors, each holding its own mysteries and dangers. You must tread carefully, for the shadows may hold more than just dust and cobwebs.")
    print("You can feel the weight of the manor's history pressing down on you, urging you to uncover its hidden truths.")
    wait_for_space()
    character_creation()
...
def show_initialization_progress():
    import sys
    import time
    print()
    steps = [
        "Generating suspects and assigning motives, alibis, and traits...",
        "Placing suspects in random rooms...",
        "Polishing monocles for maximum deduction power...",
        "Generating manor map and connecting rooms...",
        "Creating walls and locked chests...",
        "Distributing clues and artifacts throughout the manor...",
        "Assigning potions and special items to rooms...",
        "Setting up supernatural events and flavor text...",
        "Politely summoning any outerworldly entities...",
        "Preparing achievement tracking and scoring..."
    ]
    spinner = ['|', '/', '-', '\\']
    total = len(steps)
    for i, step in enumerate(steps, 1):
        ...
        sys.stdout.flush()
        # Animate progress bar and spinner for each step
        for pct in range(0, 101, 20):
            spin = spinner[(pct // 20) % len(spinner)]
            sys.stdout.write(f"\r{step} [{pct:3d}%] {spin}")
            sys.stdout.flush()
            time.sleep(0.08)
        sys.stdout.write(f"\r{step} [100%] ✓\n")
        sys.stdout.flush()
        time.sleep(0.15)
    print()
    print(f"{CYAN}Initialization complete. The investigation begins!{RESET}")
    wait_for_space()

def character_creation():
    """
    Handles new character creation, stat assignment, and game setup.
    Ensures clues are assigned to room POSITIONS (not names) and clue_locations is populated after the map is generated.
    """
    global previous_menu_function
    ...
    if bg == "1" or bg.lower() == "theologian":
        ...
        game_state["gender"] = random.choice(["Male", "Female"])
    ...
    game_state["inventory"].append("Dreaming Note") 
    ...
    # Ensure the Bishop's last location is present on the map
    ...
    # --- CRITICAL: Assign clues to room POSITIONS after map is generated ---
    assign_clues_to_rooms()
    assign_potions_to_rooms()
    assign_elder_sign_to_room()
    assign_artifacts_to_rooms()
    place_silver_key()

    show_initialization_progress()
    start_first_case()
...
while True:
    ...
    if next_choice == "1":
        # Only show difficulty menu if not already set
        if "difficulty" not in game_state or not game_state["difficulty"]:
            ...
            while True:
                ...
                if diff == "1":
                    ...
                    game_state["show_suspicion_hints"] = True
                    print("\nEasy Mode Selected:")
                    ...
                    delay_print(" - Murder board and deduction tree show alibis, trust levels, and legend for each suspect")
                    wait_for_space("Press SPACE to continue.")
                    break
                elif diff == "2":
                    ...
                    delay_print(" - Murder board and deduction tree do not show alibis or trust levels for each suspect")
                    wait_for_space("Press SPACE to continue.")
                    break
                else:
