...
def load_game():
    ...
    try:
        if os.path.exists("savegame.json"):
            ...
            # Convert lists back to sets of tuples for set-based keys
            ...
            # Always convert searched_rooms if present
            ...
            required_keys = [
                ("searched_rooms", set()),
                ("suspects_interrogated", set()),
                ("show_suspicion_hints", False),
                ("supernatural_rooms", []),
                ("score_log", []),
                ("persuasion_targets", set()),
                ("walls", set()),
                ("locked_doors", set()),
                ("breadcrumbs", set()),
                ("passages", {}),
                ...
                ("bishop_location_announced", False),
                ("bishop_last_location", ""),
                ("bishop_last_time", ""),
            ]
            for key, default in required_keys:
                if key not in game_state:
                    game_state[key] = default
            print("Loaded game_state keys:", list(game_state.keys()))
            print("Game loaded.")
            wait_for_space()
        ...
        print(f"Error in describe_room: {e}")
        import traceback
        traceback.print_exc()
        wait_for_space()
        title_screen()
...
def describe_room():
    ...
    # --- Show accusation hint if eligible ---
    ...
    # Recalculate suspects_here in case of state changes
    suspects_here = [
        s for s in game_state["suspects"]
        if s.get("position") == game_state["position"]
    ]
