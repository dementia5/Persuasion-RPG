...
def timing_bullseye_chase(rounds=3, escape_distance=3):
    ...
    while True:
        ...
        if distance >= escape_distance:
            print("You have escaped! The monster gives up the chase and slinks away into the shadows. In a panic you run through the manor, finding a safe place to hide im a previously visited room.")
            # Transport player to a random previously visited room (not current)
...
def move_suspects():
    ...
    If a suspect is transformed and has 'chasing_player', they will move toward the player."""
    for suspect in game_state["suspects"]:
...
def skill_check_combat(enemy_name, enemy_difficulty=None, stat=None):
    ...
    while player_stamina > 0 and enemy_stamina > 0:
        if round_num % 2 == 1 and round_num > 1:
            clear()
        ...
        if action == "u":
            ...
            round_num += 1  # Increment melee round after using an item
